...
This section will present some templated wrappers around functions, to provide some additional usefulness. It's not part of \autoref{functiontemplates} because it uses struct templates and it's not part of \autoref{structemplates} because the wrapper struct is not the main focus of attention.
...
Another useful transform on functions is to \emph{curry} them,\footnote{from Haskell Curry, the guy who formalized the idea.}: to transform a $n$-args function into $n$ one-parameter functions inside another.
...
\section{Generic Maker Function}
