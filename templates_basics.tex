...
Line 4 is the most interesting: with some \D{is} magic, \DD{U} has been deduced by the compiler and is accessible inside the \D{static if} branch. We use it to peel one level of `\DD{[]}' off the type and recurse\index{recursion} downward, using \DD{U} as a new type for instantiating \DD{rank}. Either \DD{U} is itself an array (in which case the recursion will continue) or it will hit the base case and stop there.
...
\item They can also be tagged with \D{@safe}, \D{@trusted} and \D{@system}. \D{@safe} means a function cannot corrupt memory. A \D{@trusted} function can call \D{@safe} ones, but offers no other guaranty concerning memory. And a \D{@system} function may do what it pleases and makes no guarantees.
\item The last property is \D{nothrow} which means you guarantee the function will not throw any exceptions.
\end{itemize}
...
\item[empty:] Which returns \D{true} if the range has no more elements, \D{false} otherwise.
\end{description}
...
Here we will begin by creating a simple input range\index{range} that takes a range of ranges\index{range!of ranges} and iterate on the inner elements. Let's begin with the very basics:
...
It's the same principle than for structs (\ref{innerstructs}). You can define inner classes using the template parameters. You can even give them method templates that use other template arguments. There is really nothing different from inner structs.
