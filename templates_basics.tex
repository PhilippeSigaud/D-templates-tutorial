...
\index{anonymous function template}
\begin{dcode}
...
\unfinished{Almost there, though.}
...
\subsection{Modifying Functions: Expanding a function}

\unfinished{I want to put some small function-wrapping templates: making a function that accepts more args than the orginal, or accepting arguments of a different type (that's useful when mapping on tuples, like in section \ref{tuplesassequences}).}
...
\index{static if@\D{static if}!nested static ifs@nested \D{static if}s}
\begin{dcode}
auto flatten(Range)(Range range)
{ 
    static if      (rank!Range == 0)
